...
namespace Moxyrus\LaravelQueueRabbitMQ;
...
use Psy\Util\Json;
use Throwable;
use Moxyrus\LaravelQueueRabbitMQ\Queue\RabbitMQQueue;
...
//    protected array $documents;
    ...
    protected int $lastPushTimestamp;
...
//                dump($message->get('priority'), json_decode($message->getBody()));
            },
            null,
            $arguments
        );
        ...
        while ($this->channel->is_consuming()) {
            ...
            $timeDiff = time() - $this->resetScope;
            if ($timeDiff >= $options->sleep && count($this->messages) > 0) {
                $this->processMessages($connection, $options, $connectionName, $queue, $jobClass, $jobsProcessed);
            } else {
                sleep(1);
            }
    ...
    protected function processMessages($connection, $options, $connectionName, $queue, $jobClass, &$jobsProcessed): void
    {
        ...
        foreach ($this->messages as $message) {
            $jsonMessage = json_decode($message->getBody());
            $jobParams = unserialize($jsonMessage->data->command);
            foreach ($jobParams->documents as $document)
                $documents[] = $document;
        }
        ...
        $jobParams->documents = $documents;
        $jsonMessage->data->command = serialize($jobParams);
        $message->setBody(Json::encode($jsonMessage));
