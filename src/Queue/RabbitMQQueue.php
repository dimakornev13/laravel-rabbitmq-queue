...
namespace Moxyrus\LaravelQueueRabbitMQ\Queue;
...
use Throwable;
use Moxyrus\LaravelQueueRabbitMQ\Queue\Jobs\RabbitMQJob;
use function Psy\sh;
...
//        $channel->close();
        ...
        $ch = curl_init("http://localhost:15672/api/queues/%2f/{$queue}");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_ANY);
        curl_setopt($ch, CURLOPT_USERPWD, "guest:guest");

        $size = json_decode(curl_exec($ch));
        curl_close($ch);
